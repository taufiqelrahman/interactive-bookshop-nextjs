...
export const retrieveInfo = (order: any) => {
  ...
  const lineItems = order.line_items.map((item: Order) => ({
    ...item,
    customAttributes: mapKeyValue(item.properties || []),
  }));
  const hasDedication = lineItems.some((item: Order) => !!item.customAttributes.Dedication);
...
export const previewImg = (item: Child) => getPreviewUrl(item);
