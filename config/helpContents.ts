export default [
  {
    id: 1,
    title: 'Mengapa harus dalam bentuk buku cerita?',
    content:
      'Memang, sangat mudah mengalihkan perhatian anak agar diam dengan gadget. Namun sayang, bonding/ ikatan orang tua - anak akan susah terbentuk. Belum lagi radiasi dan sinar biru yang dihasilkannya kurang baik bagi kesehatan anak. Karena itu, buku cerita WIGU (When I Grow Up) hadir agar orang tua-anak dapat berinteraksi bersama. Dengan berdongeng, Anda bisa ajak si kecil untuk masuk ke dalam cerita sebagai pemeran utama dan menikmati petualangan tentang cita-citanya. Dengan membaca bersama, tingkat kegemaran membaca anak bertambah. Sehingga, pengetahuannya makin luas, imajinasinya berkembang, konsentrasinya makin tajam, kemampuan sosial-emosinya terasah, dan masih banyak manfaat lainnya.',
  },
  {
    id: 2,
    title: 'Untuk anak usia berapakah buku cerita ini?',
    content:
      'Anak-anak usia 0-8 tahun dapat menikmati ceritanya yang aspiratif dan sederhana, dengan bahasa yang mudah dimengerti, dan ilustrasi yang penuh warna. Dengan format buku yang dibuat khusus (personalisasi), Anda dapat mendongengkan cerita, mengajaknya mengeja bersama, atau bahkan membiarkannya membaca sendiri jika mereka sudah bisa.',
  },
  {
    id: 3,
    title: 'Mengapa harus mengenalkan cita-cita sejak dini?',
    content:
      'Keinginan memiliki mimpi/ cita-cita tinggi perlu dikenalkan pada anak sejak dini. Salah satu bentuk cita-cita adalah profesi. Melalui buku cerita ini, Anda bisa mengenalkan keseruan dunia sesuai profesi yang diminati, sekaligus membuka wawasannya. Tentu saja, sebagaimana juga orang tua, cerita ini tetap membebaskan dan mendukung si kecil untuk memilih apa pun cita-citanya nanti.',
  },
  {
    id: 4,
    title: 'Apa saja yang bisa dipersonalisasi/ dibuat khusus untuk anak saya dalam buku cerita ini?',
    content:
      'Dalam cerita ini, anak Anda akan menjadi tokoh utamanya. Anda dapat melihatnya dalam judul cerita, nama karakter, ilustrasi karakter, bahasa,  dan 3 cita-cita pilihannya. Anda juga akan dapat menuliskan pesan khusus di akhir cerita.',
  },
  {
    id: 4,
    title: 'Apa saja cita-cita yang bisa dipilih?',
    content:
      'Si kecil dapat memilih (atau Anda bisa ikut memilihkan) 3 profesi di dalam 1 cerita. Ada 8 macam profesi yang bisa dipilih, yaitu Astronot, Guru, Dokter, Musisi, Koki, Balerina, Pilot, dan Polisi.',
  },
];
